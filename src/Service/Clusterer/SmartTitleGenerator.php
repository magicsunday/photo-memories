 ...
 */
final class SmartTitleGenerator implements TitleGeneratorInterface
{
    ...
    private function render(string $template, ClusterDraft $cluster): string
    {
        ...
        $p['date_range'] = $p['date_range'] ?? $this->formatRange($p['time_range'] ?? null);
        $p['start_date'] = $p['start_date'] ?? $this->formatDate(($p['time_range']['from'] ?? null));
        $p['end_date']   = $p['end_date']   ?? $this->formatDate(($p['time_range']['to'] ?? null));

        return \preg_replace_callback('/\{\{\s*([a-zA-Z0-9_\.]+)\s*\}\}/', static function ($m) use ($p): string {
            $key = $m[1];
            $val = $p[$key] ?? null;
            if (\is_scalar($val)) {
                return (string) $val;
