...
final class SlideshowVideoGeneratorTest extends TestCase
{
    ...
    public function testTitleOverlayIsAppliedOnlyToFirstSlide(): void
    {
        ...
        self::assertStringContainsString('scale=1280:720:force_original_aspect_ratio=increase,zoompan=z=if(gte(iw/ih\,1.778)\,1.05+(1.15-1.05)*min(t/3.75\,1)\,1)', $filterComplex);
    ...
    public function testZoompanExpressionsRespectConfiguredValues(): void
    {
        ...
        self::assertStringContainsString('zoompan=z=if(gte(iw/ih\,1.778)\,1.2+(1.3-1.2)*min(t/', $filterComplex);
        self::assertStringContainsString('x=if(gte(iw/ih\,1.778)\,clip((iw-zoom*w)/2 + 0.4*(iw-zoom*w)/2*min(t/', $filterComplex);
        self::assertStringContainsString('y=if(gte(iw/ih\,1.778)\,clip((ih-zoom*h)/2 + -0.25*(ih-zoom*h)/2*min(t/', $filterComplex);
        self::assertStringContainsString('s=1280x720', $filterComplex);
    }
