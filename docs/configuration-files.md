...
- Sämtliche Story-Profile (z. B. `vacation_weekend_transit`, `location`, `scene`, `device`, `this_day_month`, `people_friends`, `highlights`) definieren den kompletten Optionssatz, den `SelectionProfileProvider` erwartet – inklusive `min_spacing_seconds`, `phash_min_hamming`, `max_per_staypoint`, `max_per_staypoint_relaxed`, `video_bonus`, `face_bonus`, `selfie_penalty`, `quality_floor`, `minimum_total`, `enable_people_balance`, `people_balance_weight` und `repeat_penalty`.
- Das Standortprofil (`location`) hebt die Zielmenge auf 50 Medien an, erlaubt fünf Picks pro Tag und arbeitet mit Drei-Stunden-Slots sowie 1.800 Sekunden Mindestabstand. Die Qualitätsuntergrenze steigt auf `quality_floor` = 0,55; `video_bonus` = 0,24 und `face_bonus` = 0,32 sorgen für ein ausgewogeneres Verhältnis zwischen Clips und Porträts.
- Das Highlights-Profil (`highlights`) kuratiert jetzt 34 Medien bei `phash_min_hamming` = 9, reduziert die Selfie-Strafung auf 0,24 und stärkt Videos (`video_bonus` = 0,38) sowie Gesichter (`face_bonus` = 0,36), um Storys mit starker Dynamik zu bevorzugen.
